...
**TaskMaster2** to fullstack-owa aplikacja webowa do zarządzania zadaniami zespołowymi z izolacją multi-tenant (workspaces). Stack: Flask 3 (backend) + React 18 + TypeScript (frontend), SQLite jako baza, Docker + Nginx jako runtime.
...
```
                  Internet
                     ↓ HTTPS (443)
            ┌────────────────────┐
            │   Nginx (Alpine)   │  ← SSL/TLS, rate limiting, security headers
            │   reverse proxy    │
            └─────────┬──────────┘
                      ↓ HTTP (5000)
            ┌─────────────────────┐
            ...
            └─────────┬───────────┘
                      ↓
            ┌─────────────────────┐
            │  SQLite (volume)    │  ← instance/tasks.db
            └─────────────────────┘
```

Wszystko żyje w sieci `app-network` Docker Compose. Frontend SPA jest budowany w czasie buildu obrazu i serwowany jako statyki przez Flaska z `frontend/dist/`.
...
- Nginx (Alpine) jako reverse proxy
- SQLite (single-writer, do ~20 concurrent users)
...
Patrz [DEPLOYMENT.md § Team Workspaces](DEPLOYMENT.md#-team-workspaces-multi-tenancy).
...
```bash
git clone https://github.com/your/taskmaster2.git
cd taskmaster2
cp .env.example .env
# edytuj .env: SECRET_KEY, CORS_ORIGINS, DEFAULT_ADMIN_PASSWORD
./scripts/setup-ssl.sh taskmaster.local admin@taskmaster.local
docker compose up -d --build
```

Aplikacja: `https://localhost`. Login: `admin` / hasło z `.env`.
...
Migracje uruchamiają się automatycznie z `start.sh`.

### Backup

```bash
docker compose exec -T web cp /app/instance/tasks.db /app/instance/tasks.db.$(date +%F)
scp instance/tasks.db.$(date +%F) backup-host:/backups/
```
...
Na poziomie Nginx (`nginx/conf.d/taskmaster.conf`):
- `/auth/login`, `/auth/signup` — 5 requestów/min.
- `/api/*` — 30 r/s burst 30.
- `/socket.io` — 50 r/s burst 50.
- General — 10 r/s burst 20.
...
Backend musi rozpoznawać HTML navigation vs API fetch. `app.py::serve_spa` + `errorhandler(404)` zwracają `index.html` dla `Accept: text/html`. Jeśli to nie działa, sprawdź czy app.py został zaktualizowany i kontener przebudowany (`docker compose up -d --build`).

### Port 5000/443 zajęty

```bash
# Linux
lsof -i :443 ; lsof -i :5000
# Windows
...
1. Sprawdź czy backend działa: `curl -k https://localhost/health`.
2. Sprawdź `CORS_ORIGINS` w `.env`.
3. DevTools → Network → WS frame.
4. Upewnij się że Nginx przekazuje `Upgrade` header (już skonfigurowane w `nginx/conf.d/taskmaster.conf`).

### Migracja zawiesza się / błąd

```bash
# Sprawdź aktualną rewizję
...
| [DEPLOYMENT.md](DEPLOYMENT.md) | Pełny deployment + procedura migracji |
