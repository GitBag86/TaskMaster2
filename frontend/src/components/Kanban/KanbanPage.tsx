    ...
    <div className="space-y-5 page-enter">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Przeciągnij zadanie między kolumnami, aby zmienić status.</p>
        </div>
      ...
      <div className="flex gap-4 overflow-x-auto pb-2">
        {columns.map(column => {
          ...
          return (
            <section
              key={column.key}
              className={`min-w-[290px] flex-1 rounded-xl border border-border border-t-4 p-3 ${column.border} ${column.bg} ${activeColumn === column.key ? 'ring-2 ring-primary/40' : ''}`}
              onDragOver={event => {
                event.preventDefault()
              ...
              <div className="space-y-2.5">
                {columnTasks.length === 0 ? (
                  ...
                  columnTasks.map(task => (
                    ...
                    >
                      <p className={`mb-2 text-sm font-semibold ${task.completed ? 'line-through text-muted-foreground' : 'text-gray-900 dark:text-white'}`}>
                        {task.title}
                      </p>
                      ...
                      <div className="mb-2 flex items-center justify-between text-xs text-muted-foreground">
                        ...
                        {task.due_date && (
                          <span className={isOverdue(task.due_date, task.completed) ? 'font-medium text-destructive' : ''}>
