...
export default function CalendarPage() {
  ...
  const calendarWeeks = useMemo(() => {
    ...
    if (currentWeek.length > 0) {
      const lastMonthDay =
        [...currentWeek].reverse().find((day) => day !== null)?.day ??
        daysInMonth;
      weeks.push({
        weekNumber: getIsoWeekNumber(new Date(year, month, lastMonthDay)),
        days: [
          ...currentWeek,
          ...Array.from({ length: 7 - currentWeek.length }, () => null),
        ],
      });
    }
  ...
  const monthTasksCount = useMemo(() => {
    ...
    return filteredTasks.filter((task) => task.due_date?.startsWith(prefix))
      .length;
  }, [filteredTasks, month, year]);
  ...
  const monthHolidayCount = useMemo(() => {
    let total = 0;
    for (let day = 1; day <= daysInMonth; day += 1) {
      total += getPolishCalendarInfo(new Date(year, month, day)).holidays
        .length;
    }
    return total;
  ...
  return (
    <div className="space-y-5 page-enter">
      ...
      <div className="flex flex-wrap gap-2 text-xs">
        ...
        />
        <Legend
          label={`Terminów w miesiącu: ${monthTasksCount}`}
          className="bg-primary/10 text-primary"
        />
        <Legend
          label={`Świąt w miesiącu: ${monthHolidayCount}`}
          className="bg-rose-100 text-rose-700 dark:bg-rose-900/30 dark:text-rose-300"
        />
        <Legend
          label={`Po terminie: ${overdueCount}`}
      ...
      <div className="flex flex-col gap-3 rounded-lg border border-border bg-card p-3 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex flex-col gap-2 sm:flex-row sm:flex-wrap">
          <select
            ...
            onChange={(event) =>
              setPriorityFilter(event.target.value as "" | Task["priority"])
            }
                ...
                {week.days.map((calendarDay, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* Zadania - kropki na mobile, badge na desktop */}
          ...
          <div className="mb-3 flex items-start justify-between gap-3">
            <div>
              <h3 className="mb-1 text-sm font-semibold text-gray-900 dark:text-white">
                {selectedDay
                  ? `Plan dnia: ${selectedDay} ${months[month].toLowerCase()}`
                  : "Wybierz dzień"}
              </h3>
              <p className="text-xs text-muted-foreground">
                Kliknij zadanie, aby otworzyć szczegóły.
              </p>
            </div>
...
function taskDotClass(task: Task) {
...
function weekdayHeaderClass(weekday: number) {
  if (weekday === 5)
    return "bg-amber-100/80 text-amber-700 dark:bg-amber-900/20 dark:text-amber-300";
  if (weekday === 6)
    return "bg-red-100/80 text-red-700 dark:bg-red-900/20 dark:text-red-300";
...
function dayCellClass(weekday: number, isToday: boolean, isSelected: boolean) {
  ...
  if (weekday === 5)
    return "bg-amber-50/70 hover:bg-amber-100/80 dark:bg-amber-950/20 dark:hover:bg-amber-900/30";
  if (weekday === 6)
    return "bg-red-50/70 hover:bg-red-100/80 dark:bg-red-950/20 dark:hover:bg-red-900/30";
