...
import type { DashboardStats, DependencyBoardResponse, Task, WeeklyReport } from '@/types'
...
export default function DashboardPage() {
  ...
  const [dependencyBoard, setDependencyBoard] = useState<DependencyBoardResponse | null>(null)
  ...
  useEffect(() => {
    if (!lastTaskEvent || !dependencyRefreshActions.has(lastTaskEvent.action)) return
    void fetchDashboard()
    ...
    <section className="rounded-lg border border-border bg-card p-4">
      <div className="mb-4 flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground">{report.range.from} - {report.range.to}</p>
        </div>
      ...
      <div className="mt-4 grid gap-3 lg:grid-cols-2">
        <div className="rounded-lg border border-border p-3">
          <h4 className="mb-2 text-xs font-semibold uppercase tracking-wide text-muted-foreground">Najbardziej aktywne projekty</h4>
            ...
            <div className="space-y-2">
              {topProjects.map(([name, data]) => (
                <div key={name} className="flex items-center justify-between gap-3 text-sm">
                  ...
                  <span className="text-xs text-muted-foreground">{data.completed}/{data.total} zakończone, {data.open} otwarte</span>
                </div>
              ))}
            </div>
          )}
        ...
        <div className="rounded-lg border border-border p-3">
          <h4 className="mb-2 text-xs font-semibold uppercase tracking-wide text-muted-foreground">Zakończenia wg osób</h4>
            ...
            <div className="space-y-2">
              {Object.entries(report.completed_by_user).map(([username, count]) => (
                <div key={username} className="flex items-center justify-between gap-3 text-sm">
                  ...
                  <span className="rounded bg-primary/10 px-2 py-0.5 text-xs font-semibold text-primary">{count}</span>
                </div>
              ))}
            </div>
          )}
        ...
        <DependencyColumn
          ...
          renderItem={task => (
            <TaskBoardItem
              key={task.id}
              task={task}
              meta={task.blocked_by.length > 0 ? `Czeka na: ${task.blocked_by.map(blocker => blocker.title).join(', ')}` : 'Czeka na zależność'}
              tone="warning"
            />
          )}
        />
          ...
          renderItem={task => (
            <div key={task.id} className="rounded-md border border-border p-3">
              ...
              <p className="line-clamp-2 text-xs text-muted-foreground">
                Blokuje: {task.blocking_tasks.map(blockedTask => blockedTask.title).join(', ')}
              </p>
            </div>
          )}
        />
...
function DependencyColumn<T>({ title, emptyText, items, renderItem }: {
  title: string;
  emptyText: string;
  items: T[];
...
function TaskBoardItem({ task, meta, tone }: { task: Task; meta: string; tone: 'success' | 'warning' }) {
  ...
  return (
    <div className={`rounded-md border border-l-4 border-border p-3 ${toneClass}`}>
      <div className="mb-1 flex items-start justify-between gap-2">
        <p className="min-w-0 truncate text-sm font-semibold text-gray-900 dark:text-white">{task.title}</p>
